-- Complete Notification System for UT Marketplace
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    listing_id INTEGER, -- Reference to the listing if applicable
    
    -- Foreign key constraints (commented out - enable if you want strict referential integrity)
...
BEGIN
    -- Get listing details
    SELECT user_id, title, price, images[1] 
    INTO listing_owner, listing_title, listing_price, listing_image
    FROM listings 
    WHERE id = NEW.listing_id;
    
    -- Get actor name
    SELECT display_name, email INTO actor_name
    FROM user_settings 
...
BEGIN
    ...
    PERFORM create_notification(
        ...
        p_listing_id := NEW.listing_id::INTEGER
    );
